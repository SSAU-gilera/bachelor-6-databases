...
namespace Lab_6
{
    public partial class Attitude : Form
    {
        public Attitude()
        {
            InitializeComponent();
        ...
        private void buttonUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                connection.Open();
                SqlCommand command = new SqlCommand("SELECT * FROM Attitude", connection);
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            try
            {
                    ...
                    !string.IsNullOrWhiteSpace(textBox1Insert.Text) && !string.IsNullOrWhiteSpace(textBox2Insert.Text))
                {
                    SqlConnection connection = new SqlConnection("Server= LAPTOP-235LH2MO\\SQLEXPRESS; Database = Registration of incident; User ID=sa; Password=mssslkova");
                    connection.Open();
                    SqlCommand command = new SqlCommand("INSERT INTO Attitude(ID_attitude,Name_attitude) VALUES(@1, @2)", connection);
        ...
        private void buttonChange_Click(object sender, EventArgs e)
        {
            try
            {
                    ...
                    !string.IsNullOrWhiteSpace(textBox1Update.Text) && !string.IsNullOrWhiteSpace(textBox2Update.Text) && !string.IsNullOrWhiteSpace(textBox3Update.Text))
                {
                    SqlConnection connection = new SqlConnection("Server= LAPTOP-235LH2MO\\SQLEXPRESS; Database = Registration of incident; User ID=sa; Password=mssslkova");
                    connection.Open();
                    SqlCommand command = new SqlCommand("UPDATE [Attitude] SET ID_attitude = @2,Name_attitude = @3 WHERE ID_attitude = @1", connection);
        ...
        private void buttonDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (!string.IsNullOrEmpty(textBox1Delete.Text) && !string.IsNullOrWhiteSpace(textBox1Delete.Text))
                {
                    SqlConnection connection = new SqlConnection("Server= LAPTOP-235LH2MO\\SQLEXPRESS; Database = Registration of incident; User ID=sa; Password=mssslkova");
                    connection.Open();
                    SqlCommand command = new SqlCommand("DELETE FROM Attitude WHERE ID_attitude = @id", connection);
